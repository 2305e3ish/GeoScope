...
def gemini_search():
    user_query = request.get_json().get('query', '').strip() if request.method == "POST" else request.args.get('query', '').strip()
    if not user_query:
        return jsonify({'error': 'Query is required.'}), 400
    try:
        keywords = extract_keywords_gemini(user_query)
        datasets = search_nasa_cmr(keywords)
        summary_data = summarize_data_gemini(datasets)
        return jsonify({'originalQuery': user_query, 'extractedKeywords': keywords, 'datasets': datasets, 'summary': summary_data})
    except Exception as e:
